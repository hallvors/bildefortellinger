...
window.addEventListener('load', function(){
	var forms = document.getElementsByTagName('form'), theForm;
	for(var i = 0; i < forms.length; i++) {
		theForm = forms[i];
		console.log(theForm, theForm.elements.length, theForm.hasAttribute('data-jsform'), theForm.hasAttribute('data-danger'))
		if (theForm.hasAttribute('data-jsform')) {
			theForm.addEventListener('submit', function(event) {
				...
				} else if (event.target.hasAttribute('data-triggeredit')) {
					event.preventDefault();
					event.target.removeAttribute('data-triggeredit');
					event.target.setAttribute('data-edited', true);
					var elements = event.target.getAttribute('data-elements')
						.split(/,\s*/g)
						.map(function(id) {return document.getElementById(id)});
					for(var j = 0; j < elements.length; j++) {
						if (elements[j].disabled) {
							elements[j].disabled = false;
							elements[j].setAttribute('data-defaultdisabled', true);
						}
					}
					var btn = event.target.getElementsByTagName('button')[0];
					if (btn) {
						btn.setAttribute('data-defaulttext', btn.textContent);
						btn.textContent = '💾';
					}
...
function jsSubmit(event, form) {
	...
	xhr.onreadystatechange = function() {
		...
		for(var i = 0; i < form.elements.length; i++){
			form.elements[i].disabled = form.elements[i].getAttribute('data-defaultdisabled') === 'true';
		}
		var elements = form.getAttribute('data-elements')
			.split(/,\s*/g)
			.map(function(id) {return document.getElementById(id)});
		for(var i = 0; i < elements.length; i++) {
			elements[i].disabled = elements[i].getAttribute('data-defaultdisabled') === 'true';
		}
...
function formDataToPayload(form) {
	...
	for(var i = 0; i < form.elements.length; i++) {
		if (form.elements[i].name && !form.elements[i].disabled) {
			payload.push(encodeURIComponent(form.elements[i].name) +
	...
	for(var i = 0; i < elements.length; i++) {
		if (elements[i].name && !elements[i].disabled) {
			payload.push(encodeURIComponent(elements[i].name) +
